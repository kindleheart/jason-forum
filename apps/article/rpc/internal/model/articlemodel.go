...
var (
	cacheForumArticleArticleIdPrefix = "cache:forumArticle:article:id:"
)
...
func (m *customArticleModel) UpdateArticleStatus(ctx context.Context, id int64, status int) error {
	forumArticleArticleIdKey := fmt.Sprintf("%s%v", cacheForumArticleArticleIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (sql.Result, error) {
		query := fmt.Sprintf("update %s set status = ? where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, status, id)
	}, forumArticleArticleIdKey)
